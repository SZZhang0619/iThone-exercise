...
describe('AppComponent', () => {
  ...
  describe('Integration testing', () => {
    ...
    describe('the insured fields', () => {
      ...
      beforeEach(() => {
        const nameContorl = new FormControl('', [
          Validators.required,
          Validators.minLength(2),
          Validators.maxLength(10)
        ]);
      ...
      describe('the age field', () => {
        ...
        describe('Error Messages', () => {
          let ageFomrControl: FormControl;
          beforeEach(() => {
            ageFomrControl = formGroup.get('age') as FormControl;
          });

          it('should be empty string when property "pristine" of the "formControl" is `true`', () => {
            // Arrange
            const targetElement = compiledComponent.querySelector(`#${key} + .error-message`);
            // Assert
            expect(targetElement.textContent).toBe('');
          });
    ...
    describe('submit button', () => {
      let buttonElemnet: HTMLButtonElement;

      beforeEach(() => {
        buttonElemnet = compiledComponent.querySelector('button[type="submit"]') as HTMLButtonElement;
      });

      it('should be existing', () => {
        // Assert
        expect(buttonElemnet).toBeTruthy();
      });

      it('should be disabled when there are not any insureds', () => {
        // Assert
        expect(buttonElemnet.hasAttribute('disabled')).toBe(true);
      });
      ...
      describe('When there is a insured', () => {
        ...
        it('should be enabled when there ara any verifying errors that insured\'s data', () => {
          ...
          fixture.detectChanges();
          // Assert
          expect(buttonElemnet.hasAttribute('disabled')).toBe(false);
        });
